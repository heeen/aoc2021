...
fn main() -> Result<(), Box<dyn Error>> {
    let content = fs::read_to_string("day22/input")?
        .lines()
        .map(|l| {
...
/*
            for rem in &remainder {
                for on in &on_ranges {
                    assert!(rem.intersection(on).is_none());
                }
            }
*/
...
/*
            for still in &still_on{
                assert!(still.intersection(&r).is_none());
            }
*/
            on_ranges = still_on;
        }
